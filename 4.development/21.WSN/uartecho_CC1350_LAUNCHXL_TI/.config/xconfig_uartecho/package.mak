...
.dlls,em3 .dlls: uartecho.pem3

-include package/cfg/uartecho_pem3.mak
-include package/cfg/uartecho_pem3.cfg.mak
ifeq (,$(MK_NOGENDEPS))
-include package/cfg/uartecho_pem3.dep
endif
uartecho.pem3: package/cfg/uartecho_pem3.xdl
	@


ifeq (,$(wildcard .libraries,em3))
uartecho.pem3 package/cfg/uartecho_pem3.c: .libraries,em3
endif

package/cfg/uartecho_pem3.c package/cfg/uartecho_pem3.h package/cfg/uartecho_pem3.xdl: override _PROG_NAME := uartecho.xem3
package/cfg/uartecho_pem3.c: package/cfg/uartecho_pem3.cfg
package/cfg/uartecho_pem3.xdc.inc: package/cfg/uartecho_pem3.xdl
package/cfg/uartecho_pem3.xdl package/cfg/uartecho_pem3.c: .interfaces

clean:: clean,em3
	-$(RM) package/cfg/uartecho_pem3.cfg
	-$(RM) package/cfg/uartecho_pem3.dep
	-$(RM) package/cfg/uartecho_pem3.c
	-$(RM) package/cfg/uartecho_pem3.xdc.inc

clean,em3::
	-$(RM) uartecho.pem3
.executables,em3 .executables: uartecho.xem3

uartecho.xem3: |uartecho.pem3

-include package/cfg/uartecho.xem3.mak
uartecho.xem3: package/cfg/uartecho_pem3.oem3 
	$(RM) $@
	@$(MSG) lnkem3 $@ ...
	$(RM) $(XDCCFGDIR)/$@.map
	$(ti.targets.arm.elf.M3.rootDir)/bin/armcl -fs $(XDCCFGDIR)$(dir $@). -q -u _c_int00 --silicon_version=7M3 -z --strict_compatibility=on  -o $@ package/cfg/uartecho_pem3.oem3   package/cfg/uartecho_pem3.xdl  -w -c -m $(XDCCFGDIR)/$@.map -l $(ti.targets.arm.elf.M3.rootDir)/lib/libc.a
	
uartecho.xem3: export C_DIR=
uartecho.xem3: PATH:=$(ti.targets.arm.elf.M3.rootDir)/bin/;$(PATH)
uartecho.xem3: Path:=$(ti.targets.arm.elf.M3.rootDir)/bin/;$(PATH)

uartecho.test test,em3 test: uartecho.xem3.test

uartecho.xem3.test:: uartecho.xem3
ifeq (,$(_TESTLEVEL))
	@$(MAKE) -R -r --no-print-directory -f $(XDCROOT)/packages/xdc/bld/xdc.mak _TESTLEVEL=1 uartecho.xem3.test
else
	@$(MSG) running $<  ...
	$(call EXEC.uartecho.xem3, ) 
endif

clean,em3::
	-$(RM) $(wildcard .tmp,uartecho.xem3,*)
...
%,copy:
	...
	$(RM) $@
	$(CP) $< $@
uartecho_pem3.oem3,copy : package/cfg/uartecho_pem3.oem3
uartecho_pem3.sem3,copy : package/cfg/uartecho_pem3.sem3
